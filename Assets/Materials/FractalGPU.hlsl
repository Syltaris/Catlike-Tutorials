...
/*
We do this because from now on we'll use the color's A channel to set the smoothness, which is possible because we don't use it for transparency.
 This means that in our shader graph we'll use a Split node to extract the alpha channel from FractalColor and link that to the Smoothness input of Fragment.
  Then remove the smoothness property from the blackboard.
*/
